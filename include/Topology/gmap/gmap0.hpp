...
inline bool GMap0::foreach_dart_of_vertex(Dart d, FunctorType& f, unsigned int UNUSED(thread))
{
	return f(d) ;
}

inline bool GMap0::foreach_dart_of_edge(Dart d, FunctorType& f, unsigned int UNUSED(thread))
{
	if (f(d)) return true;
	Dart d1 = beta0(d);
	if (d1 != d) return f(d1);
