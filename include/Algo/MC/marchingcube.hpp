...
template< typename  DataType, template < typename D2 > class Windowing, typename PFP >
void MarchingCube<DataType, Windowing, PFP>::createLocalFaces(const unsigned char _ucCubeIndex, const int _lX, const int _lY, const int UNUSED(_lZ),  unsigned int  const *_lVertTable, const unsigned short _usMask, float UNUSED(curv), unsigned char UNUSED(tag))
{
