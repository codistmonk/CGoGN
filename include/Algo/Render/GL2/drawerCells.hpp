...
template<typename PFP>
void drawerVertices(Utils::Drawer& dr, typename PFP::MAP& UNUSED(map), std::vector<Dart>& vd, const VertexAttribute<typename PFP::VEC3>& positions)
{
	dr.begin(GL_POINTS);
...
template<typename PFP>
void drawerVertex(Utils::Drawer& dr, typename PFP::MAP& UNUSED(map), Dart d, const VertexAttribute<typename PFP::VEC3>& positions)
{
	dr.begin(GL_POINTS);
