...
#include "shaderTextureBlurH.vert"
#include "shaderTextureBlurH.frag"
...
void ShaderTextureBlurH::activeTexture(CGoGNGLuint texId)
{
	glActiveTexture(GL_TEXTURE0 + m_texUnit);
	glBindTexture(GL_TEXTURE_2D, *texId);
}

void ShaderTextureBlurH::setBlurSize(GLfloat blurSize)
{
	m_blurSize = blurSize;
	this->bind();
...
unsigned int ShaderTextureBlurH::setAttributePosition(VBO* vbo)
{
	m_vboPos = vbo;
	return bindVA_VBO("VertexPosition", vbo);
}

unsigned int ShaderTextureBlurH::setAttributeTexCoord(VBO* vbo)
{
	m_vboTexCoord = vbo;
